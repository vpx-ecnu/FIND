...
from reward_model import ValueInit
...
class MyModel(torch.nn.Module):
    def __init__(self):
        ...
        # 初始化mean和std为模型的可学习参数
        self.mean = nn.Parameter(torch.zeros(1, 4, 64, 64))
        self.std = nn.Parameter(torch.ones(1, 4, 64, 64))

    def forward(self, judge, sample=None):
        if judge==1:
            # 使用mean和std作为分布参数
            ...
            return self.get_logprob(sample)
    
    def get_logprob(self, sample):
        self.check_std()
        if (self.std<0).any():
          pdb.set_trace()
        dist = Normal(self.mean, self.std)
        log_prob = (
            dist.log_prob(sample.detach().clone())
            .mean(dim=-1)
            .mean(dim=-1)
            .mean(dim=-1)
        )
        
        kl = (self.mean-torch.zeros_like(self.mean)) ** 2 + (self.std-torch.ones_like(self.std)) ** 2
        return log_prob, kl
      
    def check_std(self):
      # self.std = self.std.detach()  # 将 requires_grad 设置为 False
      self.std.data.clamp_(min=0.0001)
      # self.std.requires_grad = True  # 恢复 requires_grad 设置为 True


def get_log_prob(mean,std,sample):
...
def get_random_indices(num_indices, sample_size):
  ...
  """
  return np.random.choice(num_indices, size=sample_size, replace=True)
...
def _update_output_dir(args):
  ...
  if args.single_flag == 1:
    data_log = "single_prompt/" + truncate_string(args.single_prompt).replace(" ", "_") + "/"
  else:
...
def _save_model(args, count, is_ddp, accelerator, myModel, pipe, prompt):
    """Saves UNET model."""
    save_path = os.path.join(args.output_dir, f"save_{count}")
    current_datetime = datetime.now()
    datetime_string = current_datetime.strftime("%m-%d-%H:%M:%S")
    folder_path = f"{save_path}/{datetime_string}"
    os.makedirs(folder_path, exist_ok=True)
    
    print(f"Saving model to {save_path}")
    torch.save(myModel.mean.detach().cpu(), f"{folder_path}/mean.pt")
    torch.save(myModel.std.detach().cpu(), f"{folder_path}/std.pt")
    
    dist = Normal(myModel.mean.detach(), myModel.std.detach())
    img_path = save_path.replace("save", "image")
    if not os.path.exists(img_path):
        os.makedirs(img_path)
    for i in range(10): 
        init_noise = dist.sample()
        images = pipe(prompt=prompt, latents=init_noise.to(dtype=torch.half), eta=1.0).images
        image = images[0]
...
def calculate_log_prob(mean, std, mean_copy, std_copy, sample):
    # pdb.set_trace()
    kl_regularizer = (mean - mean_copy) ** 2 + (std - std_copy) ** 2
    dist = Normal(mean, std)
    log_prob = (
        dist.log_prob(sample.detach().clone())
        .mean(dim=-1)
...
def _train_policy_func(
  ...
  with torch.no_grad():
    indices = np.array([0])
    # pdb.set_trace()
    init_noise = state_dict["init_noise"]
    ...
    batch_txt_emb = state_dict["txt_emb"][indices]
    batch_log_prob = state_dict["last_log_prob"]
...
#   )
  ...
  input = torch.cat((myModel.mean.detach(), myModel.std.detach()), dim=1)
  with torch.no_grad():
    if args.v_flag == 1:
...
#   init_noise_copy = init_noise_copy.detach()
  ...
  value_function = ValueInit((2, 4, 64, 64))
  ...
  state_dict["last_log_prob"] = _map_cpu(torch.FloatTensor().to(weight_dtype))
  state_dict["last_log_prob"] = torch.tensor([0]).cuda()


  state_dict["old_mean"] = myModel.mean.data.clone()
  state_dict["old_std"] = myModel.std.data.clone()
  # pdb.set_trace()
        ...
        if v_step < args.v_step-1:
          with accelerator.no_sync(value_function):
            tot_val_loss += _train_value_func(
                myModel, value_function, state_dict, accelerator, args
            )
        else:
          tot_val_loss += _train_value_func(
              myModel, value_function, state_dict, accelerator, args
...
#   tpfdata.tot_grad_norm += norm.item() / args.p_step
    ...
    lr_scheduler.step()
    if accelerator.is_main_process:
        print(f"count: [{count} / {args.max_train_steps}]")
    ...
    if count % args.save_interval == 0:
      ...
      if accelerator.is_main_process:
        _save_model(args, count, is_ddp, accelerator, myModel, pipe, args.single_prompt)
