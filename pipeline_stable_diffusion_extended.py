...
class StableDiffusionPipelineExtended(StableDiffusionPipeline):
  ...
  def forward_calculate_logprob(
    ...
    if init_noise_copy is not None:
      
      latent_model_input_copy = (
        torch.cat([init_noise_copy] * 2) if do_classifier_free_guidance else init_noise_copy
      )
      
      old_noise_pred = self.unet(
          latent_model_input_copy.to(dtype=torch.float16),
          time_model_input,
          encoder_hidden_states=prompt_embeds,
          cross_attention_kwargs=cross_attention_kwargs,
      ).sample
